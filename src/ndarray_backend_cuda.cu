...
// 内核函数指示了gpu中每个线程应该执行的操作
__global__ void FillKernel(scalar_t* out, scalar_t val, size_t size) {
  size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
  if (gid < size) out[gid] = val;
}

// cpu上的Fill函数调用gpu上的内核函数，达到并行执行的目的
...
__global__ void MatmulKernel(const scalar_t* a, const scalar_t* b, scalar_t* out, size_t size, size_t M, size_t N, 
                             size_t P) {
