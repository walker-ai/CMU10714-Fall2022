...
void Compact(const AlignedArray& a, AlignedArray* out, std::vector<int32_t> shape,
  ...
  std::vector<size_t> pos(dim, 0);  // pos[]里面存的是当前每一个维度的索引，并且是从后往前存储的，比如pos[0]存的是最后一个维度的索引,pos[dim - 1]存的是第一个维度的索引
...
void Matmul(const AlignedArray& a, const AlignedArray& b, AlignedArray* out, uint32_t m, uint32_t n,
  ...
  for (size_t i = 0; i < m; i ++ ) 
    for (size_t k = 0; k < p; k ++ ) {
      scalar_t sum = 0;  // 这里最好使用中间变量sum，这里是因为out内的元素有可能没有初始化
...
inline void AlignedDot(const float* __restrict__ a,
  ...
  for (size_t i = 0; i < TILE; i ++ )
    for (size_t k = 0; k < TILE; k ++ ) {
      for (size_t j = 0; j < TILE; j ++ ) { // 这里不需要中间变量将out置为0，因为这是在计算一个瓦片中的值，需要累加
